...
export async function getFriendNote(accessToken: string, friendChildId: string): Promise<{ myNote: string | null; theirNote: string | null }> {
  return request<{ myNote: string | null; theirNote: string | null }>(`/api/v1/child/friends/${friendChildId}/note`, withAuth(accessToken))
}
